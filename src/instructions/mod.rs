...
pub const INSTRUCTIONS: [(&'static str, fn(&mut CPU, &mut MMU) -> u8); 256] = [
    ...
    ("LD (0xFF00 + N), A", load::ld_at_n_a),   // 0xe0
    ("POP HL", pop_hl),                        // 0xe1
    ("LD (0xFF00 + C), A", load::ld_at_c_a),   // 0xe2
    ...
    ("LD A, (0xFF00 + N)", load::ld_a_from_n), // 0xf0
    ("POP AF", pop_af),                        // 0xf1
    ("LD A, (0xFF00 + C)", load::ld_a_from_c), // 0xf2
    ("DI", di),                                // 0xf3
...
fn jr_cc(cpu: &mut CPU, mmu: &mut MMU, condition: bool) -> u8 {
    let n = cpu.fetch_byte(mmu) as i8 as u16;
    if condition {
        cpu.regs.pc += n;
        return 12;
    }
...
fn call_cc(cpu: &mut CPU, mmu: &mut MMU, condition: bool) -> u8 {
    ...
    if condition {
        cpu.regs.sp -= 2;
        mmu.write_word(cpu.regs.sp, cpu.regs.pc);
        cpu.regs.pc = nn;
        return 24;
    }
    12
...
fn ret(cpu: &mut CPU, mmu: &mut MMU) -> u8 {
    ret_cc(cpu, mmu, true);
    16
}
